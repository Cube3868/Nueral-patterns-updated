...
export default {
    ...
    methods: {
        ...
        refreshSymmetryFlagsAndInputs() {
            this.hor_sym = Controller.hor_sym;
            this.ver_sym = Controller.ver_sym;
            this.full_sym = Controller.full_sym;

            // This logic is adapted from setSymmetry to update input disabled states
            let disabled = [];
            if (this.full_sym) {
                disabled.push(...[2, 3, 5, 6, 7, 8]); // Simplified, full_sym disables most
            }
            if (this.hor_sym) {
                disabled.push(...[6, 7, 8]);
            }
            ...
            for (let i = 0; i < 9; i++) {
                if (this.$refs.filterInputs && this.$refs.filterInputs[i]) {
                    this.$refs.filterInputs[i].disabled = disabled.includes(i);
                }
            }
            // Important: Also re-apply the (potentially new) filter to UI from controller
            // as randomizeAllParameters would have changed it.
            this.setFilter(Controller.filter);
        },
    }
}
