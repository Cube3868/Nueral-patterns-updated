...
class Renderer {
	...
	setZoomLevel(level) {
		...
		if (this.gl && this.onePixelAttr) { // Ensure GL context and attribute are available
			this.applyValues(); // Re-apply uniforms with new zoom
		}
	}

	applyValues(){
		let gl = this.gl;
		gl.uniform2f(this.onePixelAttr, (1/this.width) * this.zoomLevel, (1/this.height) * this.zoomLevel);
